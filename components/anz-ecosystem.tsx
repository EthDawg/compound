...
import { capabilityColumns, careerAtCompany, type Ecosystem, type AnzCompany, type Fact } from '@/lib/data/anz-ecosystem';
import { MarketMovement, CompanyMovementReading, MovementBadges, MovementMethod } from './anz-movement';
import type { MovementFilter } from '@/lib/data/market-movement';
...
const MOVEMENTS: MovementFilter[] = ['Building', 'Growing', 'Acquired', 'Contracting', 'Mixed signals'];
...
function Signal({ id }: { id: string }) {
  ...
  return <MovementBadges reading={companyMovement(id)} quiet />;
}

function EcosystemView() {
  const data = useEcosystem();
  const { CAPABILITIES, ANZ_PEOPLE, searchAnz, anzCompany, ecosystemHref, companyPeople } = data;
  ...
  const selected = anzCompany(params.get('firm') ?? '') ?? (view === 'movement' ? undefined : visible[0]);
  ...
  useEffect(() => {
    ...
    if (person || window.matchMedia(view !== 'people' ? '(max-width: 1279px)' : '(max-width: 1023px)').matches) destination?.scrollIntoView({ block: 'start', behavior: 'auto' });
  }, [params.get('firm'), params.get('person'), view]);
    ...
    <main className="mx-auto max-w-[1440px] px-4 pb-16 sm:px-6">
      ...
      <div className={`grid items-start gap-4 ${view !== 'people' ? 'xl:grid-cols-[minmax(0,1fr)_350px]' : 'lg:grid-cols-[minmax(250px,.75fr)_minmax(0,1.65fr)]'}`}>
        <section className="min-w-0">
          ...
          {view === 'movement' && <MarketMovement data={data} visible={visible} focusedFirm={focusedFirm} query={query} cap={cap} movement={movement} history={history} href={href} />}
          {view !== 'movement' && !visible.length && <Empty />}
        </section>
        <div ref={detail} tabIndex={-1} className="min-w-0 scroll-mt-36 outline-none sm:scroll-mt-20">{selected && !selected.historical && !visible.some((c) => c.id === selected.id) && <p className="mb-2 rounded-lg bg-white p-3 text-xs text-slate-500">This profile is outside the current filters. <Link href={ecosystemHref({ lens: view, firm: selected.id })} className="text-[var(--eco-link)] underline">Clear filters</Link></p>}{selected ? <CompanyDetail company={selected} view={view} personId={focusedPerson} href={href} /> : view === 'movement' ? <MovementMethod data={data} /> : null}</div>
      </div>
      <details className="mt-6 rounded-xl border border-slate-200 bg-white p-4 text-xs leading-relaxed text-slate-500"><summary className="cursor-pointer font-semibold text-slate-700">How to read this research</summary><div className="mt-3 max-w-4xl space-y-2"><p>This is a selected ANZ intelligence layer, not a ranking or a complete census. Sources stay beside the facts. “Our reading” is Compound’s interpretation, not a company claim.</p><p>Movement readings use qualified, dated evidence within 18 months of 11 September 2026. ANZ operating direction, APAC context and ownership changes are separate. Opposing local signals are shown as mixed; a newer positive event does not erase contraction. Only explicit growth evidence supports Growing. An acquisition applies to its target, not its buyer. Undated or insufficiently precise timing stays outside recent events.</p><p>One departure does not establish contraction. No recent signal means insufficient dated evidence here. Size remains unestablished where public material does not isolate the ANZ {data.name} practice. {data.note}</p><p>Customer examples establish the named engagement only. Career steps are selected milestones; gaps are not filled with assumed employment or ownership relationships.</p></div></details>
    </main>
  </div>;
}
...
function CompanyDetail({ company: c, view, personId, href }: { company: AnzCompany; view: string; personId: string | null; href: (v: Record<string, string | undefined>) => string }) {
  const data = useEcosystem();
  const { peopleInLineage, ANZ_LINEAGES, ANZ_CUSTOMERS, ANZ_EVENTS, anzCompany } = data;
  ...
  return <article className="overflow-hidden rounded-xl border border-slate-200 bg-white">
    ...
    <div className="space-y-5 p-5 sm:p-6">
      {view === 'movement' && <CompanyMovementReading data={data} company={c} href={href} />}

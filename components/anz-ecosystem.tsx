...
function EcosystemView() {
  ...
  const shownGroups = columns === 'all' ? columnGroups : [columnGroups[columns === 'services' ? 0 : columnGroups.length - 1]];
  const shownCaps = shownGroups.flatMap((g) => g.values);
      ...
      <div className={`grid items-start gap-4 ${view !== 'people' ? 'xl:grid-cols-[minmax(0,1fr)_350px]' : 'lg:grid-cols-[minmax(250px,.75fr)_minmax(0,1.65fr)]'}`}>
        <section className="min-w-0">
          ...
          {view === 'capability' && <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div className="border-b border-slate-100 p-4"><h2 className="text-sm font-semibold">Compare the role, then the domain.</h2><p className="mt-2 text-xs leading-relaxed text-slate-500">Service roles describe how a firm works. Domains describe what it works on. A tick in each does not prove every combination.</p><div className="mt-3 flex flex-wrap items-center gap-2">{[{id:'services',name:'Service roles'},{id:'domains',name:data.id==='workday'?'Product domains':'Workflow domains'},{id:'all',name:'All columns'}].map((g) => <Link key={g.id} href={href({columns:g.id})} aria-current={columns===g.id?'true':undefined} className={`rounded-md px-3 py-2 text-xs font-semibold ${columns===g.id?'bg-[var(--eco-soft)] text-[var(--eco-link)]':'bg-slate-50 text-slate-500'}`}>{g.name}</Link>)}<label className="ml-auto"><span className="sr-only">Evidence threshold</span><select value={evidence} onChange={(e)=>update({evidence:e.target.value,firm:undefined})} className="max-w-full rounded-md border border-slate-200 bg-white px-2 py-2 text-xs"><option value="">All evidence</option><option value="local">ANZ offers or cases</option><option value="named">Named ANZ customers</option></select></label></div><p className="mt-3 text-[11px] leading-relaxed text-slate-500"><strong className="text-emerald-700">✓</strong> Named case · <strong className="text-emerald-700">◐</strong> Anonymous case · <strong className="text-[var(--eco-link)]">●</strong> ANZ offer · <strong>◇</strong> Credentials only · <strong>G</strong> Regional/global · — unknown</p></div>
            <div className="overflow-x-auto"><table className={`w-full text-left text-xs ${columns==='all'?'min-w-[800px]':'min-w-[540px]'}`}><caption className="sr-only">{data.name} ANZ capabilities and evidence. Select a cell to inspect the source in its company profile.</caption><thead className="bg-slate-50">{columns==='all'&&<tr><th/>{shownGroups.map((g)=><th key={g.label} colSpan={g.values.length} className="border-l border-slate-200 px-2 py-2 text-center text-[10px] font-semibold">{g.label}</th>)}</tr>}<tr><th scope="col" className="sticky left-0 z-10 min-w-[140px] bg-slate-50 p-3">Practice</th>{shownCaps.map((c)=><th scope="col" key={c} className="max-w-[95px] p-2 text-center text-[10px] font-medium text-slate-500">{c==='Data / Integration / AI'?'Data / Int. / AI':c}</th>)}</tr></thead><tbody>{visible.map((c)=><tr key={c.id} className={`border-t border-slate-100 ${selected?.id===c.id?'bg-[var(--eco-soft)]':''}`}><th scope="row" className="sticky left-0 bg-white p-3 font-semibold"><Link href={href({firm:c.id})} className="text-[var(--eco-link)] hover:underline">{c.name}</Link></th>{shownCaps.map((capability)=>{
              const item=data.capabilityEvidence(c.id,capability);
              return <td key={capability} className="p-1 text-center">{item.key==='unknown'?<span className="text-slate-300" aria-label={item.label}>—</span>:<Link href={href({firm:c.id})} aria-label={`${c.name}, ${capability}: ${item.label}`} title={item.label} className={`inline-block rounded p-2 font-bold hover:bg-blue-100 ${['named','anonymous'].includes(item.key)?'text-emerald-700':item.key==='local'?'text-[var(--eco-link)]':'text-slate-400'}`}>{item.mark}</Link>}</td>;
            })}</tr>)}</tbody></table></div><p className="border-t border-slate-100 p-3 text-[10px] leading-relaxed text-slate-500">Evidence filters keep firms with at least one matching visible domain, or the capability selected above. A blank cell means this research has not established the claim.</p>
          </div>}

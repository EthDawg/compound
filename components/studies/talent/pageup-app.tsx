...
import { usePageUpReview } from "./pageup-review";
...
function Home() {
  const [{reviewed}]=usePageUpReview();
  return <>
    <PageHeading eyebrow="Hiring manager workspace" title="The next step needs a person.">{reviewed ? "Your committee review is recorded. The hiring manager can now consider the recommendation; no offer has been authorised." : "Your optical engineering shortlist is ready for committee review. One review is still outstanding before the hiring manager can proceed."}</PageHeading>
    <div className={s.grid}>{[
      ["Jobs", "2", reviewed ? "One role with the hiring manager" : "One role at committee review", "recruitment"], ["Applications", "3", "Candidates for optical engineering", "recruitment"], ["Advertisements", "1", "Careers campaign ready to preview", "talent"],
      ["Selection committee", reviewed ? "0" : "1", reviewed ? "Your review is recorded" : "Review waiting on you", "recruitment"], ["Interviews", "2", "Completed; evidence ready", "paige"], ["Offers", "0", reviewed ? "Hiring manager decision next" : "Review must finish first", "recruitment"],
...
function Recruitment() {
  ...
  return <>
    ...
    <div className={s.two}><div className={s.stack}>
      ...
      {selected === 0 ? <><label className={s.field}>Your review note<textarea className={s.input} value={note} onChange={e => updateReview({note:e.target.value})} maxLength={2000} rows={3} disabled={reviewed} placeholder="Record the evidence and what still needs checking"/></label><div className={s.actions}><button className={s.button} disabled={reviewed || note.trim().length < 10} onClick={() => updateReview({reviewed:true})}>{reviewed ? "Review recorded" : "Record demo review"}</button>{reviewed && <button className={s.secondary} onClick={() => updateReview({reviewed:false,note:""})}>Reset review</button>}</div><p className={s.small}>Use at least 10 characters. Your note stays in this browser session across PageUp screens. Recording a review does not hire or reject anyone.</p></> : <p className={s.callout}>Your review is already recorded for this applicant. {reviewed ? "The committee's reviews are complete; the hiring manager acts next." : "Ava's outstanding review is the committee's next action."}</p>}
    </section></div>
...
function Paige() {
  const [query, setQuery] = useState("evidence"), [selected, setSelected] = useState(0), [citation, setCitation] = useState(false);
  const [{reviewed,saved},updateReview]=usePageUpReview();
  const p = TALENT[selected];
  return <>
    ...
    {query === "waiting" ? <section className={s.card}><p className={s.label}>Paige · response</p><h2 className="mt-3">{reviewed ? "Your review is recorded. The hiring manager acts next." : "Your committee review for MO-204 is outstanding."}</h2><p className={s.muted}>{reviewed ? "Ava’s review note is recorded in MO-204. The hiring manager can consider the committee’s recommendation; an offer is still a separate decision." : "Ava’s application needs your review note. Completing it lets the hiring manager consider the recommendation; it does not authorise an offer."}</p><p className={s.small}>Source: fictional requisition MO-204 · the same review state used by Applications and the hiring dashboard.</p><Link className={`${s.button} mt-5`} href={companyHref("pageup", "app", "recruitment")}>Open application review</Link></section> : <div className={s.two}><section className={s.card}><p className={s.label}>Paige · candidate evidence</p><h2 className="mt-3">Relevant experience, with gaps still visible.</h2><p className={s.muted}>The records support different aspects of the role. They do not establish a complete hiring recommendation.</p><div className={s.tabs}>{TALENT.map((x, i) => <button key={x.id} aria-pressed={selected === i} onClick={() => {setSelected(i);setCitation(false);}}>{x.name}</button>)}</div><h3>{p.name}</h3><ul className={s.checklist}><li><strong>Evidence present</strong><br/>{p.fit}</li><li><strong>Still to verify</strong><br/>{p.gap}</li></ul><button className={`${s.link} mt-4`} aria-expanded={citation} onClick={() => setCitation(!citation)}>Inspect source [1] →</button><div className={s.actions}><button className={s.button} disabled={saved.includes(p.id)} onClick={() => updateReview({saved:[...saved,p.id]})}>{saved.includes(p.id) ? "Added to your review list" : "Add to demo review list"}</button></div><p className={s.small} role="status">{saved.length} added · visible in Applications; a human review step, not a selection outcome</p></section>

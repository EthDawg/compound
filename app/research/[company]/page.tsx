...
import { ResearchWayfinding } from '@/components/research-wayfinding';
...
export default async function ResearchPage({params}:{params:Promise<{company:string}>}){
  ...
  return <div className="destination min-h-screen bg-[#F7F6F1] text-[#17282C]"><GuideHeader activeId={company.id}/><main className="mx-auto max-w-[1100px] px-4 pb-16 sm:px-6"><AtlasMaps active="categories"/>
    <Suspense fallback={<div className="h-[76px]"/>}><ResearchWayfinding companyId={company.id} name={company.name} position="breadcrumb"/></Suspense>
    ...
    <div className="mt-7 flex flex-wrap justify-between gap-4 text-xs text-ink-500"><Suspense fallback={<span className="min-h-11"/>}><ResearchWayfinding companyId={company.id} name={company.name} position="landscape"/></Suspense><a href={`https://github.com/EthDawg/compound/issues/new?title=${encodeURIComponent(`Research correction: ${company.name}`)}`} className="inline-flex min-h-11 items-center underline underline-offset-4">Suggest a correction ↗</a></div>

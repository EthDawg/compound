...
import { RESEARCH_CATEGORIES, RESEARCH_REVIEWED, researchCategory, companiesInResearch, researchHref, type ResearchCompany, type ResearchCategoryId } from '@/lib/data/category-research';
...
function CompanyRow({company, context}:{company:ResearchCompany; context:ResearchCategoryId}) {
  return <Link href={researchHref(company.id,context)} className="group grid gap-3 border-t border-[#E3E3DC] py-5 sm:grid-cols-[220px_1fr_auto] sm:gap-5">
...
export default async function CategoryPage({params}:{params:Promise<{category:string}>}) {
  ...
  return <DestinationShell active="categories"><Link href="/categories" className="mt-5 inline-flex min-h-11 items-center text-xs text-ink-500 hover:underline">← All category guides</Link><DestinationIntro kicker={`Category guide · ${companies.length} companies`} title={category.name} body={category.thesis}/>
    ...
    <section aria-labelledby="category-companies"><div className="mb-6 flex flex-wrap items-baseline justify-between gap-3"><h2 id="category-companies" className="font-serif-display text-2xl">Know who does what.</h2><p className="text-[11px] text-ink-500">Reviewed {RESEARCH_REVIEWED} · Curated, not exhaustive</p></div>
      {category.groups.map(group=><section key={group.id} className="mb-7"><div className="mb-3 flex flex-wrap items-baseline gap-x-4 gap-y-2"><h3 className="text-sm font-semibold">{group.name}</h3><p className="text-xs text-ink-500">{group.detail}</p></div>{companies.filter(c=>c.categories[0]===category.id&&c.group===group.id).map(c=><CompanyRow key={c.id} company={c} context={category.id}/>)}</section>)}
      {!!adjacent.length&&<section className="mb-7"><h3 className="mb-3 text-sm font-semibold">Across the category boundary</h3>{adjacent.map(c=><CompanyRow key={c.id} company={c} context={category.id}/>)}</section>}
    </section>

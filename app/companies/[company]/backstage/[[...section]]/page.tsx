...
import Library from "@/components/studies/rippling/backstage/page";
...
export default async function CompanyBackstagePage({params}:Props){
  ...
  if(BACKSTAGE_SECTIONS.some(s=>s.id===key))content=<CompanyBackstage company={c} section={key}/>;
  else if(c.id==='rippling'){
    const Screen=RIPPLING_BACKSTAGE_SCREENS[key];
    if(key==='library')content=<Library/>;

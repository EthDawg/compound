...
export const RESEARCH_CATEGORIES: ResearchCategory[] = [
  {
    ...
    boundary: 'Follow the route from a model to a running service. An API, a custom serving stack and a GPU cluster leave different work with the customer. Some companies span several layers.',
    groups: [
      ...
      {id:'packaged-runtime', name:'Packaged inference software', detail:'Deploy a supported model runtime on infrastructure you choose and operate.'},
    ...
    situations: [
      ...
      {id:'latency', name:'Make an agent faster', title:'Find the slow step before buying faster inference.', explanation:'Sequential tool calls, retrieval and network hops can dominate the wait. First establish support for the model and features your task needs; then measure the portion a faster decoder could improve.', steps:['Confirm model + feature support','Separate model and tool time','Test speed at equal quality'], check:'Hold model quality and concurrency constant. Measure p95 task time, rather than comparing unrelated tokens-per-second claims.', companies:['groq','cerebras','sambanova','fireworks']},
      {id:'boundary', name:'Choose what we operate', title:'A model page is the start of several different routes.', explanation:'A provider router chooses where a request runs. A dedicated endpoint manages a deployment. A packaged runtime goes onto infrastructure you operate. Similar APIs can conceal different ownership of capacity, upgrades and recovery.', steps:['Model + deployment constraints','Choose who operates each layer','Test failure and recovery'], check:'Name the execution provider, region and rollback owner. Disable the preferred route: does it fail closed, switch providers, or require your team to restore capacity?', companies:['hugging-face','nvidia','baseten','modal']},
    ],
  },
  {
...
export const RESEARCH_COMPANIES: ResearchCompany[] = [
  {
    id:'nvidia',name:'NVIDIA',product:'NIM · NIM Certified · NVIDIA AI Enterprise',categories:['ai-inference','enterprise-ai'],group:'packaged-runtime',accent:'#436B08',monogram:'N',terms:['NIM','NIM Certified','CUDA','Jensen Huang','Chris Malachowsky','Curtis Priem','TensorRT','Dynamo','AI Enterprise'],
    thesis:'The hardware advantage extends into the software that makes an inference service operable.',
    strengths:['Package supported models, inference engines and APIs in deployable containers.','Run those containers on supported NVIDIA infrastructure across cloud and data centre environments.','Choose a release and support lifecycle to match how the service will be maintained.'],
    productSource:s('NVIDIA · NIM product and deployment boundary','https://www.nvidia.com/en-us/ai-data-science/products/nim-microservices/'),
    productSources:[s('NVIDIA · current NIM offerings','https://docs.nvidia.com/nim/large-language-models/latest/day-0/index.html')],
    origin:{text:'Jensen Huang, Chris Malachowsky and Curtis Priem founded NVIDIA in 1993 for 3D graphics. CUDA opened GPU parallel processing to scientific computing in 2006. NIM builds on that hardware-and-software lineage.',sources:[s('NVIDIA · corporate timeline','https://www.nvidia.com/en-gb/about-nvidia/corporate-timeline/')]},
    movement:{date:'4 Sep 2026 · documentation snapshot',title:'Model access and lifecycle support are separate choices.',text:'The current LLM documentation distinguishes rapid-access NIM from NIM Certified, with Feature and Production Branch options. Hardware validation, update cadence and support differ by offering.',implication:'Compound’s reading: the exact container and maintenance contract matter more than a generic “NVIDIA-powered” label. This is a current product distinction, not a launch-date claim.',sources:[s('NVIDIA · offerings and lifecycle distinctions','https://docs.nvidia.com/nim/large-language-models/latest/day-0/index.html')]},
    tradeoff:'A packaged runtime still leaves infrastructure, capacity, access and recovery to an operator. Check the model licence, supported hardware, image tag and support terms for the chosen offering.',
    watch:'Whether new models reach the supported branch your organisation can maintain, and whether upgrades improve the complete workload.',
    related:[{id:'hugging-face',label:'Hugging Face · distribution and managed routes',href:'/research/hugging-face'},{id:'coreweave',label:'CoreWeave · the capacity underneath',href:'/research/coreweave'}],
  },
  {
    id:'hugging-face',name:'Hugging Face',product:'Hub · Inference Providers · Inference Endpoints',categories:['ai-inference','developer-tools'],group:'catalogue',accent:'#896308',monogram:'HF',terms:['Huggingface','Hub','Inference Providers','Inference Endpoints','PrivateLink','model repository','model weights'],
    thesis:'Owning model discovery creates a natural place to choose how—and where—the model will run.',
    strengths:['Connect model discovery and versioned repositories with paths into inference.','Call supported independent providers through a common client and routing layer.','Deploy a model on managed infrastructure through the separate dedicated Endpoints service.'],
    productSource:s('Hugging Face · Inference Providers','https://huggingface.co/docs/inference-providers/en/index'),
    productSources:[s('Hugging Face · dedicated Endpoints and operating boundaries','https://huggingface.co/docs/inference-endpoints/support/faq')],
    origin:{text:'The January 2025 provider launch described a shift from Hugging Face’s own serverless API toward connecting independent providers to the Hub. Its established collaboration, versioning and distribution layer became an entry point into other companies’ serving infrastructure.',sources:[s('Hugging Face · provider integration launch, 28 Jan 2025','https://huggingface.co/blog/inference-providers')]},
    movement:{date:'Sep 2026 · product snapshot',title:'Routing policy becomes part of the product.',text:'Current documentation supports explicit provider selection and automatic routing policies, including failover when a provider is flagged unavailable. Dedicated Endpoints remain a separate managed-deployment service.',implication:'Compound’s reading: the familiar model page can hide an important handoff. Inspect the actual execution provider and fallback route before making a reliability or location claim.',sources:[s('Hugging Face · current routing behaviour','https://huggingface.co/docs/inference-providers/en/index'),s('Hugging Face · Providers versus Endpoints','https://huggingface.co/docs/inference-endpoints/support/faq')]},
    tradeoff:'A common API does not make model, task or parameter support identical. A private Endpoints connection through AWS PrivateLink is also different from hosting the endpoint inside your own VPC.',
    watch:'Which models and tasks are supported on each route, and how predictable the execution destination remains during a provider failure.',
    related:[{id:'fireworks',label:'Fireworks · follow the serving provider',href:'/research/fireworks'},{id:'nvidia',label:'NVIDIA · run a packaged runtime',href:'/research/nvidia'}],
  },
  {
    id:'sambanova',name:'SambaNova',product:'SambaCloud · SambaStack · RDU systems',categories:['ai-inference','enterprise-ai'],group:'silicon',accent:'#2C588D',monogram:'S',terms:['Samba Nova','SambaCloud','SambaStack','RDU','SN50','Rodrigo Liang','Kunle Olukotun','Christopher Ré','Intel'],
    thesis:'A different hardware architecture matters when it becomes a usable serving system, in the cloud or inside the enterprise.',
    strengths:['Combine dataflow hardware with software designed to map model workloads onto it.','Try supported models through the SambaCloud API.','Use the SambaStack enterprise path, with product-specific capabilities that must be checked separately from cloud access.'],
    productSource:s('SambaNova · SambaCloud and SambaStack developer guide','https://docs.sambanova.ai/docs/en/get-started/overview'),
    productSources:[s('SambaNova · dataflow hardware and software lineage','https://sambanova.ai/resources/ai-is-changing-everything')],
    origin:{text:'Founded in 2017, SambaNova brings together Rodrigo Liang’s and Kunle Olukotun’s Afara/Sun processor experience with Stanford machine-learning systems research, including co-founder Christopher Ré. That lineage explains the integrated hardware-and-software approach.',sources:[s('SambaNova · founders and architecture history','https://sambanova.ai/resources/ai-is-changing-everything')]},
    movement:{date:'8 Jul 2026',title:'Capital and enterprise demand support the next stage.',text:'SambaNova announced the first close of a $1 billion Series F financing at an $11 billion post-money valuation, led by General Atlantic. It also reported JPMorganChase selecting its RDUs for on-premises inference.',implication:'Compound’s reading: financing and a named customer selection strengthen the commercial signal. They do not establish deployed capacity or independently measured customer outcomes.',sources:[s('SambaNova · financing and customer-selection announcement','https://sambanova.ai/press/sambanova-completes-first-close-of-1b-financing-at-11b-valuation?hs_amp=true')]},
    tradeoff:'An API makes the hardware easier to try, but supported models, checkpoint options and deployment arrangements still constrain the comparison. Measure the complete task at the required quality.',
    watch:'Delivery of announced systems and the proposed Intel collaboration, where GPUs, RDUs and CPUs handle different stages of the workload.',
    related:[{id:'intel-collaboration',label:'SambaNova / Intel · announced heterogeneous inference design',href:'https://sambanova.ai/press/sambanova-announces-collaboration-with-intel-on-ai-solution?hs_amp=true'}],
  },
  {
